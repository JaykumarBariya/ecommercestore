    ...
    try{
        ...
        if(body.data.id != "")
        {
            ...
            if(base64image != ""){
                ...
                product.imagepath = "product/" + randomname + ".png";
...
router.post("/list",async(req,res)=>{
    try{
        ...
        res.end(JSON.stringify({status:"success",data:product}));
            ...
            let product = await Product.find({pcid : pcid});
            res.end(JSON.stringify({status:"success",data:product})); 
        }
    }catch{
    res.end(JSON.stringify({status:"failed",data:"something went wrong"}));
...
router.post("/get",async(req,res)=>{
    try{
    let body = req.body;
    let product = await Productcategory.findById(body.data.id);
    res.end(JSON.stringify({status:"success",data:product}));
    }
    catch{
        res.end(JSON.stringify({status:"failed",data:"something went wrong"}));
